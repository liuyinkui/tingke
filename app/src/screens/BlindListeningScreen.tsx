...
export default function BlindListeningScreen({ onComplete, onExit }: Props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const msg = err?.response?.data?.error?.message || '无法加载今日素材';
        setError(msg);
      } finally {
        setLoading(false);
      }
  ...
  const handleProgress = useCallback(
    (_currentMs: number) => {
      // Mark as listened once playback starts
      if (!hasListened) {
        setHasListened(true);
      }
    },
  ...
  const handleReplay = useCallback(() => {
    // AudioPlayer handles this internally via seekTo(0)
    // This is a cue for the user experience
    setHasListened(true);
  }, []);
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.centerContent}>
          ...
          <Text style={styles.loadingText}>加载今日素材...</Text>
        </View>
      </SafeAreaView>
    );
  }
